...
export interface UseTutorialStateMachineReturn {
  ...
  handleCorrectGuess: (guess: string) => void;
...
export function useTutorialStateMachine(puzzle: Puzzle): UseTutorialStateMachineReturn {
  ...
  const handleCorrectGuess = useCallback((guess: string) => {
  ...
  const isStepRevealed = useCallback((stepId: number) => {
    return machine.isStepRevealed(stepId);
  }, [machine]);

  const isCurrentQuestion = useCallback((stepId: number) => {
    return machine.isCurrentQuestion(stepId);
  }, [machine]);

  const isCurrentAnswer = useCallback((stepId: number) => {
