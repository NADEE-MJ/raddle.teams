...
describe('useTutorialStateMachine', () => {
  ...
  describe('state updates', () => {
    it('updates state when dispatching correct guess', () => {
      ...
      act(() => {
        result.current.handleCorrectGuess('SOUTH');
      });
  ...
  describe('convenience handlers', () => {
    it('handleCorrectGuess works with valid guess', () => {
      const { result } = hookResult;
    ...
    it('handleCorrectGuess ignores invalid guess', () => {
      const { result } = hookResult;
      const initialState = result.current.state;

      act(() => {
        result.current.handleCorrectGuess('WRONG');
      });
  ...
  describe('helper method consistency', () => {
    ...
    it('canSwitchDirection updates correctly', () => {
      ...
      act(() => {
        result.current.handleCorrectGuess('SOUTH');
        result.current.handleCorrectGuess('MOUTH');
        result.current.handleCorrectGuess('TONGUE');
      });
  ...
  describe('completion flow', () => {
    it('handles complete puzzle workflow', () => {
      ...
      act(() => {
        ...
        result.current.handleCorrectGuess('MOUTH');
        result.current.handleCorrectGuess('TONGUE');
        result.current.handleCorrectGuess('SHOE');
      });
  ...
  describe('multiple dispatches', () => {
    it('handles rapid state changes correctly', () => {
      ...
      act(() => {
        result.current.handleCorrectGuess('SOUTH'); // Reveals SOUTH (step 1)
