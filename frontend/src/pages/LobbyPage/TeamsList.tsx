...
export default function TeamsList({ teams, playersByTeam, currentPlayer }: TeamsListProps) {
    return (
        <div className='rounded-lg bg-gray-50 p-4'>
            <h2 className='mb-4 text-xl font-semibold text-gray-900' data-testid='player-teams-heading'>
                Teams ({teams && teams.length > 0 ? teams.length : 0})
            </h2>
            {teams && teams.length > 0 ? (
                <div className='space-y-3'>
                    {teams.map(team => (
                        <div key={team.id} className='rounded-lg border border-gray-200 bg-white p-4' data-testid={`team-section-${team.name}`}>
