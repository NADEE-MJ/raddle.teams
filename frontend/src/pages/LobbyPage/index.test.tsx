...
vi.mock('@/components', () => ({
  ...
  CopyableCode: ({ code, 'data-testid': dataTestId }: any) => (
    <span data-testid={dataTestId}>{code}</span>
  ),
  ...
  describe('Teams Display', () => {
    ...
    test('shows empty state when no teams', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('No teams created yet. Waiting for admin to set up teams...')).toBeInTheDocument()
      })
    })
  })
