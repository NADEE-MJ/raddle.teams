"""
Player awards system - assigns fun titles based on performance.
"""
...
AWARDS_CATALOG = {
    "MVP": PlayerAward(
        key="MVP", title="Most Valuable Player", emoji="🏆", description="Most correct guesses on the team"
    ),
    "SPEED_DEMON": PlayerAward(
        key="SPEED_DEMON", title="Speed Demon", emoji="⚡", description="Fastest to submit correct guesses"
    ),
    "CLUTCH": PlayerAward(key="CLUTCH", title="Clutch Player", emoji="💪", description="Solved the final word"),
    ...
    "CREATIVE": PlayerAward(
        key="CREATIVE", title="Creative Guesser", emoji="🎨", description="Most unique wrong guesses"
    ),
    ...
    "PUZZLE_MASTER": PlayerAward(
        key="PUZZLE_MASTER", title="Puzzle Master", emoji="🧩", description="Solved the most difficult words"
    ),
    "STRATEGIST": PlayerAward(
        key="STRATEGIST", title="Strategist", emoji="🧠", description="Best guess diversity across puzzle"
    ),
    "WORD_WIZARD": PlayerAward(
        key="WORD_WIZARD", title="Word Wizard", emoji="🪄", description="Solved words in unusual order"
    ),
}


def assign_awards(team_stats: List[dict], puzzle_length: int) -> dict[int, List[PlayerAward]]:
    """
    Assign 1-3 awards per player based on performance.

    Args:
        team_stats: List of player stat dictionaries
        puzzle_length: Length of the puzzle ladder
    ...
    # Sharpshooter: Highest accuracy with minimum 5 guesses
